...
local function Notify(title, content)
    local isMatcha = pcall(function()
        return identifyexecutor and identifyexecutor():lower():find("matcha")
    end)
    if isMatcha and getgenv().notify then
        pcall(function() getgenv().notify({Title = title, Content = content, Duration = 3}) end)
    else
        pcall(function()
...
local function GetActiveUsers(callback)
    task.spawn(function()
        ...
        if success and res then
            ...
            if ok and decoded and decoded.active_users then
...
function UiLib:CreateWindow(options)
    ...
    local menuKey
    if UiLib.Config.MenuKey then
        local ok, kc = pcall(function() return Enum.KeyCode[UiLib.Config.MenuKey] end)
        menuKey = (ok and kc) or options.MenuKey or Enum.KeyCode.F1
    else
        menuKey = options.MenuKey or Enum.KeyCode.F1
    end
    ...
    local ScreenGui = Create("ScreenGui", {
        Name = "MacUiLib",
        Parent = (syn and syn.protect_gui and syn.protect_gui(Instance.new("ScreenGui"))) and CoreGui or (pcall(function() return CoreGui end) and CoreGui or Players.LocalPlayer:WaitForChild("PlayerGui")),
        ResetOnSpawn = false,
        DisplayOrder = 100
    })
    ...
    function Window:CreateTab(tabName)
        ...
        function Tab:CreateSection(secName)
            ...
            -- Slider
            function Elements:CreateSlider(sName, min, max, default, isFloat, desc, callback)
                if type(desc) == "function" then callback = desc; desc = nil end
                if type(isFloat) == "string" then desc = isFloat; isFloat = false end
                ...
                SLabel.MouseEnter:Connect(function() ShowTooltip(desc) end)
                SLabel.MouseLeave:Connect(function() HideTooltip() end)
